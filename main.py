...
while 1:
    ...
    for info in result: 
        ...
        for box in boxes:
            ...
            if (objectdetect == 'car' and conf > 75) or ((objectdetect == 'bus' or objectdetect == 'truck') and conf > 60):
                x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
                new_detections = np.array([x1, y1, x2, y2, conf])
                detections = np.vstack((detections, new_detections))

                #cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 180))
                #cvzone.putTextRect(frame, f'{classnames[classindex]} - {conf}%',
    ...
    for results in track_result:
        ...
        if line[0] < cx < line[2] and line[1] - 20 < cy < line[1] + 20:
            ...
            if counter.count(id) == 0:
                counter.append(id)
    cvzone.putTextRect(frame, f'{len(counter)}', [600, 40], thickness=2, scale=1.5)
